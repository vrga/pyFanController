...
def determine_inputs(device_config: SectionProxy) -> List[InputDevice]:
    ...
    if device_config.get('inputType') == 'componentTemp':
        for path in device_config.get('temperatureMonitor').split(', '):
            input_devices.extend(LMSensorsInput.from_path(device_config.get('temperatureMonitorDeviceName'), path))
